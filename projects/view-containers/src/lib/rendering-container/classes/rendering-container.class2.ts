...
export class RenderingContainer2<T  extends InjectableI> {
    ...
    createNewContainer(source: RendererSelector<T>):RenderingContainer2<T> {
        return new RenderingContainer2(source, this.host)
    }
    ...
    setComponentRefference(refference : ComponentRef<ContainerNodeComponent<T>>, parentViewRef : ViewContainerRef ):RenderingContainer2<T>{
        this.thisComponentsRefference = refference
        this.parentViewRef = parentViewRef
        return this
    }

    setAssets(assets: ContainerComponentAsset<ContainerNodeComponent<T>>[]): RenderingContainer2<T> {
        this.assets = assets
        this.factory
        return this
    }
    ...
    setSourceHtml(html: string): RenderingContainer2<T> {
        if (this.html != html) {
            this.html = html
